...
import { SERVIZI } from "@/app/components/ShowServizi";

export default function ServiziPage() {
    const sectionRef = useRef<HTMLElement>(null);
    useScrollReveal(sectionRef);

    // Mappatura dei dati per il Bento Grid con stili variabili per i 12 servizi
    const getBentoStyle = (index: number, categoria: string) => {
        const patterns = [
            // 0: Grande White
            {
                bentoClasses: "lg:col-span-2 lg:row-span-2 md:col-span-2 bg-white p-8 md:p-12 min-h-[400px] lg:min-h-0",
                textTitleClasses: "text-3xl md:text-5xl lg:text-6xl text-[#1a1a1a]",
            ...
            // 1: Medium Dark
            {
                ...
                bgImage: "/sfondo/sfondo1.webp",
                textTitleClasses: "text-2xl lg:text-4xl text-white",
                textDescClasses: "text-white/60 text-base",
                numeroClasses: "text-white/50",
                iconClasses: "text-white/80 w-6 h-6",
            },
            // 2: Stretto Verticale Accent (Rose)
            {
            ...
            // 4: Orizzontale Emerald Accent
            {
             ...
             // 5: Piccolo Dark
             {
                bentoClasses: "lg:col-span-1 lg:row-span-1 bg-[#1a1a1a] p-8 min-h-[250px] lg:min-h-0 text-white",
                textTitleClasses: "text-xl text-white",
                textDescClasses: "text-white/60 text-sm",
                numeroClasses: "text-white/50",
                iconClasses: "text-white/80 w-5 h-5",
            },
        ];

        // Logica di distribuzione manuale per un layout armonioso (12 servizi)
        const sequence = [0, 1, 3, 2, 3, 1, 5, 2, 4, 3, 1, 3];
        return patterns[sequence[index % sequence.length]];
    };
            ...
            <div className="flex-1 w-full max-w-[95%] mx-auto px-4 md:px-16 py-20 md:py-32">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:auto-rows-[300px]">
                    {SERVIZI.map((servizio, index) => {
                        const style = getBentoStyle(index, servizio.categoria);
                        ...
                        return (
                            <div
                                ...
                                className={`group flex flex-col justify-between relative overflow-hidden rounded-[2rem] ${style.bentoClasses}`}
                            >
                                ...
                                {style.hasBlur && (
                                    <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-48 h-48 rounded-full pointer-events-none" 
                                         style={{ 
                                             background: style.blurColor === 'rose' ? 'radial-gradient(circle, rgba(136,19,55,0.2) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(6,78,59,0.2) 0%, transparent 70%)'
                                         }} 
                                    />
                                )}
                                ...
                                <div className="z-10 relative flex justify-between items-start">
                                    <div className="flex flex-col gap-0.5">
                                        <span className={`anony text-[10px] tracking-[0.2em] uppercase ${style.numeroClasses}`}>
                                            {numero} — {servizio.categoria}
                                        </span>
                                        {servizio.prezzoLabel && (
                                            <span className={`font-bold text-xs tracking-tight ${style.numeroClasses}`}>
                                                {servizio.prezzoLabel}
                                            </span>
                                        )}
                                    </div>
                                    <Icona className={style.iconClasses} strokeWidth={1.2} />
                                </div>

                                <div className="z-10 relative flex flex-col gap-3 mt-12 lg:mt-0">
                                    <h2 className={`font-bold tracking-tight leading-[1.1] ${style.textTitleClasses}`}>
                                        {servizio.nome}
                                    </h2>
                                    <p className={`leading-relaxed max-w-sm ${style.textDescClasses}`}>
